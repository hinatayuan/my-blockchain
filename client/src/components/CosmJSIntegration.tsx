...
const CosmJSIntegration: React.FC = () => {
  ...
  const connect = async (e: React.FormEvent): Promise<void> => {
    ...
    try {
      ...
      setMessage({
        text:
          error.response?.data?.error || 'Failed to connect to Cosmos network',
        type: 'error'
      })
    } finally {
      setLoading(false)
  ...
  const sendTokens = async (e: React.FormEvent): Promise<void> => {
    e.preventDefault()
    setLoading(true)
    ...
    <div>
            ...
            <div className="stat-card">
              <div className="stat-value">
                {connectionInfo.address
                  ? connectionInfo.address.substring(0, 8)
                  : 'N/A'}
              </div>
            ...
            <form onSubmit={sendTokens}>
              ...
              <div className="form-group">
                ...
                <select
                  ...
                  onChange={(e) =>
                    setSendForm((prev) => ({ ...prev, denom: e.target.value }))
                  }
                  disabled={loading}
                >
          ...
          <div className="card">
            ...
            >
              ...
              <p>
                ...
                {balance ? `${balance.amount} ${balance.denom}` : 'Loading...'}
              </p>
              {networkInfo && (
                <>
                  <p>
                  ...
                  <p>
                    <strong>Latest Height:</strong> {networkInfo.height}
                  </p>
                </>
              )}
            </div>
      ...
      <div className="card">
        ...
        <div style={{ color: '#666', lineHeight: '1.6' }}>
          ...
          <p>
            CosmJS is a TypeScript/JavaScript library for interacting with
            Cosmos SDK-based blockchains. It provides tools for:
          </p>
          ...
          <ul style={{ paddingLeft: '1.5rem' }}>
            <li>
              <strong>Wallet Connection:</strong> Connect with existing mnemonic
              or generate new wallet
            </li>
            <li>
              <strong>Balance Checking:</strong> Query account balances on
              Cosmos Hub
            </li>
            <li>
              <strong>Token Transfers:</strong> Send ATOM tokens to other
              addresses
            </li>
            <li>
              <strong>Transaction Simulation:</strong> Estimate gas costs before
              sending
            </li>
            <li>
              <strong>Network Information:</strong> View chain ID and current
              block height
            </li>
          </ul>
          ...
          <ul style={{ paddingLeft: '1.5rem' }}>
            ...
            <li>
              <strong>Osmosis:</strong> https://rpc.osmosis.zone
            </li>
            <li>
              <strong>Juno:</strong> https://rpc.juno.strange.love
            </li>
          </ul>

          <div className="alert info" style={{ marginTop: '1rem' }}>
            <strong>Note:</strong> This integration connects to real Cosmos
            networks. Be careful with real tokens and private keys!
          </div>
        </div>
      </div>
    </div>
