...
const BlockchainExplorer: React.FC<BlockchainExplorerProps> = ({
  ...
  const getBlockByIndex = (index: number): Block | undefined => {
    return blocks[index]
  }
      ...
      <div className="card">
        ...
        <div style={{ display: 'flex', gap: '1rem', marginBottom: '1rem' }}>
          <select
            value={searchType}
            onChange={(e) =>
              setSearchType(e.target.value as 'block' | 'transaction')
            }
          ...
          <input
            ...
            placeholder={`搜索 ${
              searchType === 'block' ? '区块索引、哈希' : '交易ID、地址、类型'
            }`}
            style={{ flex: 1 }}
          />
        </div>
              ...
              {filteredBlocks.reverse().map((block, reverseIndex) => {
                ...
                return (
                  <div
                    ...
                    onClick={() =>
                      setSelectedBlock(
                        selectedBlock === actualIndex ? null : actualIndex
                      )
                    }
                  >
                    ...
                    <div style={{ marginBottom: '0.5rem' }}>
                      <strong>哈希:</strong>{' '}
                      <code>{formatHash(block.hash)}</code>
                    </div>

                    <div style={{ marginBottom: '0.5rem' }}>
                      <strong>前置哈希:</strong>{' '}
                      <code>{formatHash(block.previousHash)}</code>
                    </div>
              ...
              {filteredTransactions.reverse().map((tx, index) => (
                <div key={tx.id || index} className="transaction-item">
                  ...
                  {tx.signature && (
                    <div style={{ marginBottom: '0.3rem' }}>
                      <strong>签名:</strong>{' '}
                      <code>{formatHash(tx.signature)}</code>
                    </div>
                  )}
                </div>
              ))}
      ...
      <div className="card">
        ...
        <div style={{ color: '#666', lineHeight: '1.6' }}>
          ...
          <ul style={{ paddingLeft: '1.5rem' }}>
            <li>
              <strong>搜索区块:</strong> 通过索引、哈希或前置哈希查找区块
            </li>
            <li>
              <strong>搜索交易:</strong> 通过ID、地址或类型查找交易
            </li>
            <li>
              <strong>区块详情:</strong> 点击任意区块查看其交易
            </li>
            <li>
              <strong>交易类型:</strong>
            ...
            <li>
              <strong>哈希验证:</strong> 每个区块的哈希都是根据其内容计算得出
            </li>
          </ul>
        </div>
      </div>
