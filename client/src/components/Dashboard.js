...
const Dashboard = ({ blockchainData, wallets, notifications }) => {
  ...
  const getRecentTransactions = () => {
    ...
    return blockchainData.transactions.filter(tx => tx.timestamp > oneDayAgo).length
  }
      ...
      <div className="card" style={{ marginBottom: '1rem' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: '1rem' }}>
          <div style={{
            ...
            <div style={{ fontSize: '0.9rem', color: '#6c757d', marginBottom: '0.5rem' }}>24小时交易</div>
          ...
          <div style={{
            ...
            <div style={{ fontSize: '0.9rem', color: '#6c757d', marginBottom: '0.5rem' }}>平均区块时间</div>
          ...
          <div style={{
            ...
            <div style={{ fontSize: '0.9rem', color: '#6c757d', marginBottom: '0.5rem' }}>挖矿奖励</div>
          ...
          <div style={{
            ...
            <div style={{ fontSize: '0.9rem', color: '#6c757d', marginBottom: '0.5rem' }}>链状态</div>
            <div style={{
              fontSize: '1.5rem',
              fontWeight: 'bold',
          ...
          <div className="wallet-list">
            {wallets.slice(0, 3).map((wallet, index) => (
              <div key={index} className="wallet-card">
                ...
                {wallet.balance === 0 && (
                  <div style={{ color: '#ffc107', fontSize: '0.8rem', marginTop: '0.25rem' }}>
                    💡 可以使用水龙头获取代币
                  </div>
                )}
              </div>
      ...
      {blockchainData.pendingTransactions.length > 0 && (
        <div className="card">
          ...
          <div style={{
            ...
            <strong>💡 提示:</strong> 这些交易正在等待被挖掘。请前往挖矿页面开始挖矿以确认这些交易。
          </div>
          <div className="transaction-list">
            {blockchainData.pendingTransactions.map((tx, index) => (
              <div key={index} className="transaction-item" style={{
                ...
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                  ...
                  <div style={{ fontSize: '0.8rem', color: '#ffc107', fontWeight: 'bold' }}>
                    ⏳ 等待挖掘
                  </div>
                </div>
                {tx.from && tx.to && (
